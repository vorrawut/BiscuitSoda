...
if ( ! class_exists( 'X_TGM_Automatic_Update' ) ) :
  ...
  class X_TGM_Automatic_Update {
    ...
    function x_add_updates( $transient ) {
      ...
      foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
        if ( isset( $plugin['version'] ) && isset( $plugin['x_manage_upgrade'] ) && $plugin['x_manage_upgrade'] == true && version_compare( $installed_plugins[$plugin['x_plugin']]['Version'], $plugin['version'], '<' ) ) {
