...
if ( ! function_exists( 'x_buddypress_navbar_menu' ) ) :
  function x_buddypress_navbar_menu( $items, $args ) {
    ...
    if ( X_BUDDYPRESS_IS_ACTIVE && x_get_option( 'x_buddypress_header_menu_enable', 0 ) == 1 ) {

      $top_level_link = ( is_user_logged_in() ) ? bp_loggedin_user_domain() : bp_get_activity_directory_permalink();
      $submenu_items  = '';

      if ( bp_is_active( 'activity' ) ) {
        $submenu_items .= '<li class="menu-item menu-item-buddypress-navigation"><a href="' . bp_get_activity_directory_permalink() . '" class="cf"><i class="x-icon x-icon-thumbs-up"></i> <span>' . x_get_option( 'x_buddypress_activity_title' ) . '</span></a></li>';
      }

      if ( bp_is_active( 'groups' ) ) {
        $submenu_items .= '<li class="menu-item menu-item-buddypress-navigation"><a href="' . bp_get_groups_directory_permalink() . '" class="cf"><i class="x-icon x-icon-sitemap"></i> <span>' . x_get_option( 'x_buddypress_groups_title' ) . '</span></a></li>';
      }

      if ( is_multisite() && bp_is_active( 'blogs' ) ) {
        $submenu_items .= '<li class="menu-item menu-item-buddypress-navigation"><a href="' . bp_get_blogs_directory_permalink() . '" class="cf"><i class="x-icon x-icon-file"></i> <span>' . x_get_option( 'x_buddypress_blogs_title' ) . '</span></a></li>';
      }

      $submenu_items .= '<li class="menu-item menu-item-buddypress-navigation"><a href="' . bp_get_members_directory_permalink() . '" class="cf"><i class="x-icon x-icon-male"></i> <span>' . x_get_option( 'x_buddypress_members_title' ) . '</span></a></li>';

      if ( ! is_user_logged_in() ) {
        if ( bp_get_signup_allowed() ) {
          $submenu_items .= '<li class="menu-item menu-item-buddypress-navigation"><a href="' . bp_get_signup_page() . '" class="cf"><i class="x-icon x-icon-pencil"></i> <span>' . x_get_option( 'x_buddypress_register_title' ) . '</span></a></li>';
          $submenu_items .= '<li class="menu-item menu-item-buddypress-navigation"><a href="' . bp_get_activation_page() . '" class="cf"><i class="x-icon x-icon-key"></i> <span>' . x_get_option( 'x_buddypress_activate_title' ) . '</span></a></li>';
        }
...
if ( ! function_exists( 'x_buddypress_get_the_title' ) ) :
  function x_buddypress_get_the_title() {
    ...
    if ( x_is_buddypress_user() ) {
      ...
      $output = x_get_option( 'x_buddypress_activity_title', 'Activity' );
    } else if ( x_is_buddypress_component( 'groups' ) ) {
      if ( x_is_buddypress_group() ) {
        $output = bp_get_current_group_name();
      } else {
        $output = x_get_option( 'x_buddypress_groups_title', 'Groups' );
      }
    } else if ( x_is_buddypress_component( 'members' ) ) {
      $output = x_get_option( 'x_buddypress_members_title', 'Members' );
    } else if ( x_is_buddypress_component( 'blogs' ) ) {
      $output = x_get_option( 'x_buddypress_blogs_title', 'Sites' );
    } else if ( x_is_buddypress_component( 'register' ) ) {
      $output = x_get_option( 'x_buddypress_register_title', 'Create An Account' );
    } else if ( x_is_buddypress_component( 'activate' ) ) {
      $output = x_get_option( 'x_buddypress_activate_title', 'Activate Your Account' );
    } else {
...
if ( ! function_exists( 'x_buddypress_get_the_subtitle' ) ) :
  function x_buddypress_get_the_subtitle() {
    ...
    if ( x_is_buddypress_component( 'activity' ) ) {
      $output = x_get_option( 'x_buddypress_activity_subtitle', 'Meet new people, get involved, and stay connected.' );
    } else if ( x_is_buddypress_component( 'groups' ) ) {
      $output = x_get_option( 'x_buddypress_groups_subtitle', 'Find others with similar interests and get plugged in.' );
    } else if ( x_is_buddypress_component( 'members' ) ) {
      $output = x_get_option( 'x_buddypress_members_subtitle', 'See what others are writing about. Learn something new and exciting today!' );
    } else if ( x_is_buddypress_component( 'blogs' ) ) {
      $output = x_get_option( 'x_buddypress_blogs_subtitle', 'Meet your new online community. Kick up your feet and stay awhile.' );
    } else if ( x_is_buddypress_component( 'register' ) ) {
      $output = x_get_option( 'x_buddypress_register_subtitle', 'Just fill in the fields below and we\'ll get a new account set up for you in no time!' );
    } else if ( x_is_buddypress_component( 'activate' ) ) {
      $output = x_get_option( 'x_buddypress_activate_subtitle', 'You\'re almost there! Simply enter your activation code below and we\'ll take care of the rest.' );
    } else {
      $output = '';
    }
